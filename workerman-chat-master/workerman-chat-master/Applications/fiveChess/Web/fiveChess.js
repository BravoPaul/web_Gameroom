...
var fiveChess = {
			...
			switch (id) {
                ...
                case "replay_btn":
                    ...
                    if (gameStart==0) {	//点重玩
                        ...
                        ws.send(ready_data);
                        if(ready_adverse!=0&&gameStart==0){
                            fiveChess.init();
                            fiveChess.gameStart();
                        }
                        ready = 1;
	...
	resetChessBoard: function () {
		...
		$("div.chessboard div").click(function () {
			if (wait!=0 || _fiveChess.isGameOver||ready==0) {
				return;
			}
			...
			if (_fiveChess.chessArr[i][j] === _fiveChess.NO_CHESS) {
                ...
                var pace_data = '{"type":"pace","adverse":"'+adverse_id+'","lastPace":"'+[i,j]+'"}';
                ws.send(pace_data);
                wait = 1;
	...
	//AI下棋
	AImoveChess: function (maxX,maxY) {
