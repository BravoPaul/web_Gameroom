    ...
    public static function onMessage($client_id, $message)
    {
        ...
        switch($message_data['type'])
        {
            ...
            case 'win':
                ...
                Gateway::sendToClient($to_id,json_encode($new_message));
                // 客户端发言 message: {type:say, to_client_id:xx, content:xx}
                $client_note = $db1->select('note')->from("chess")->where("username= '".self::$name[$to_id]."'")->single();
                $client_note = $client_note+20;

                $db1->update('chess')->cols(array('note'=>''.$client_note.''))->where("username= '".self::$name[$to_id]."'")->query();
                return;
